...
let productIdArr = [...productsList].map((item) => item.getAttribute('data-id'));
...
getInfoAboutProduct = (item) => {
        ...
        console.log(quantity.forEach(el => {
                console.log(el);
        }));

        
      
        let a = quantity.forEach(item => {
        ...
        addBtn.addEventListener('click', () => {
                ...
                `;
                const idRenderElem = productIdArr.find(item => item === dataId);
                ...
                if(document.getElementById(idRenderElem)) {
                        ...
                        foundElement.querySelector('.cart__product-count').innerHTML = +countBlock.textContent + +prevCount.textContent;
                } else {
